...
- Generate a template of workflow metadata file
...
Generate a workflow metadata file template from the primary workflow file URL.
...
Validate the schema and contents of the workflow metadata file.
...
Explanation of validation rules for some fields in the workflow metadata file:
...
The tests are executed using the Workflow Execution Service (WES; [GA4GH - WES API](https://www.ga4gh.org/news/ga4gh-wes-api-enables-portable-genomic-analysis/).
...
An example of the `workflow.testing` field is as follows:
...
In addition, at WES runtime, the `other` files are placed in the execution directory with `target` as a path.
...
The flow when this command is executed is as follows:
...
Upload files to Zenodo and generate TRS responses.
